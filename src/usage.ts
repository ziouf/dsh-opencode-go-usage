...
import type { OpenCodeUsageData, OpenCodeUsageErrorCode, OpenCodeUsageWindow } from './types.ts'
...
export async function fetchOpenCodeUsage(
  ...
  try {
    ...
    throw new OpenCodeUsageFetchError(
      timeout ? 'timeout' : 'connect',
      timeout ? {} : { detail: errorMessage(error) },
 ...
 */
export function parseUsageBody(body: unknown): OpenCodeUsageData | undefined {
  if (typeof body !== 'object' || body === null) return undefined
  const usage = (body as { usage?: unknown }).usage
  if (typeof usage !== 'object' || usage === null) return undefined
  const record = usage as Record<string, unknown>
  const out: OpenCodeUsageData = {}
...
function parseWindow(value: unknown): OpenCodeUsageWindow | undefined {
